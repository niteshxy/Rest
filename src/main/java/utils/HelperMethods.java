...
public class HelperMethods {
	...
	/*
	 * Get Video Ids (For example 1) We can use get method of Rest Assured
	 * library's JsonPath Class's get method PArt of a response is shown below:
	 * "items": [{ "id": 519377522, .... We can get all id's with this code -->
	 * "jp.get("items.id");" this will return all id's under "items" tag.
	 */
	...
	/*
	 * Get Related Video Ids (For example 2) Structure of response is shown
	 * below: items: "related": [{ "id": 519148754, .... In order to get all
	 * id's under related tag, We can use JsonPath's get method like
	 * "jp.get("items.related.id");" It will give us all id's under related tag.
	 */
	public static ArrayList getRelatedVideoIdList(JsonPath jp) {
		...
		/*
		 * Result of relatedVideosList: [[519148754, 519115214, 519235328,
		 * 519235341]] I have to convert above result in this format:
		 * [519148754, 519115214, 519235328, 519235341] In order to split first
		 * element of "relatedVideosList" and assign it to a new ArrayList (as
		 * splittedRelatedVideoList) I did below operation.
		 */
