...
public class Example2Test {
	...
	/*
	 * We should do setup operations, get JSON response from the API and put it
	 * into JsonPath object Then we will do query and manipulations with
	 * JsonPath class’s methods. We can do all of the preparation operations
	 * after @Before Junit annotation.
	 */
	@Before
	public void setup() {
		// Test Setup
	...
	private void printAttributes(JsonPath jp) {
		for (int i = 0; i < getMergedVideoLists().size(); i++) {
			...
			if (jp.get("items.title[" + i + "]") != null) {
				...
				System.out.println(
						"ConnectedDevices: " + jp.get("items.permittedDeviceTypes.ConnectedDevices[" + i + "]"));
				...
				// Check that sent video has related videos? If yes print their
				// attributes
				if (jp.get("items.related.title[" + i + "][" + i + "]") != null) {
					for (int j = 0; j < HelperMethods.getRelatedVideoIdList(jp).size(); j++) {
						...
						System.out.println(
								"Tablets: " + jp.get("items.related.permittedDeviceTypes.Tablets[0][" + j + "]"));
						System.out.println(
								"Handsets: " + jp.get("items.related.permittedDeviceTypes.Handsets[0][" + j + "]"));
						System.out.println("ConnectedDevices: "
								+ jp.get("items.related.permittedDeviceTypes.ConnectedDevices[0][" + j + "]"));
						System.out.println(
								"Computers: " + jp.get("items.related.permittedDeviceTypes.Computers[0][" + j + "]"));
